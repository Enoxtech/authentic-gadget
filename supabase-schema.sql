...
-- Products: public read (active only via API)
...
CREATE POLICY "orders_read" ON public.orders FOR SELECT USING (auth.uid() = customer_id OR customer_id IS NULL);
CREATE POLICY "orders_insert" ON public.orders FOR INSERT WITH CHECK (true);
CREATE POLICY "orders_update" ON public.orders FOR UPDATE USING (true);

-- Order items: anyone can insert, read own
